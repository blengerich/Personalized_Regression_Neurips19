...
class LinearRegressionsMixture(Model):
    ...
    def __init__(self, X, y, K, lam=0, iterations=1000, epsilon=1e-4,
        random_restarts=None, fit_intercept=True, init_w=None):
        """
        :param X:               Data set
        :param y:               Labels
    ...
    def _expectation_maximization(self, X, y, verbose, lr=1e0):
        """
        ...
        if self.init_w is None:
            w = np.random.normal(0, 1, size=(D, self.K))
        else:
            w = self.init_w.copy()
        ...
        for i in range(self.iterations):
            ...
            probabilities_new = np.tile(np.sqrt(beta), (N, 1)) * np.exp(prbs)  # N(y_n | <w_k, x_n>, beta^{-1})
            probabilities = probabilities_new

            # Compute expected mixture weights
            gamma_new = np.tile(pi, (N, 1)) * probabilities
            gamma_new /= np.tile(np.sum(gamma_new, 1), (self.K, 1)).T
            gamma = gamma_new
            ...
            for k in range(self.K):
                linear_reg = LinearRegression(fit_intercept=False,
                    normalize=False)
                linear_reg.fit(tX, y, sample_weight=gamma[:, k]**2)
                w[:, k] += lr*(np.squeeze(linear_reg.coef_) - w[:, k])

            # Max with respect to the precision term
            #beta_new = N * pi / np.sum(gamma * err)
            #beta += lr*(beta_new - beta)
            ...
            complete_log_likelihood = float(np.sum(np.log(np.sum(probabilities, axis=1))))

            if verbose:
                print("{}\n{}".format(complete_log_likelihood, w))
    ...
    def _train(self, X, y, verbose, silent, init_lr):
        """
    ...
    def train(self, seed=None, verbose=False, silent=False, init_lr=1, **kwargs):
        """
        ...
        if seed:
            np.random.seed(seed)
        self.w, self.pi, self.gamma, self.beta, self.marginal_likelihood = self._train(
            self.X, self.y, verbose, silent, init_lr=init_lr)
        self.trained = True
